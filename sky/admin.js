...
console.log('showPage defined. Type:', typeof showPage);
...
function renderOpportunityCard(opportunity) {
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            <span class="applicants-count">${opportunity.max_applicants ? escapeHtml(opportunity.max_applicants + ' applicants') : '0 applicants'}</span>
...
function showEmptyOpportunityState() {
    const grid = document.getElementById('opportunitiesGrid');
    if (!grid) return;
    grid.innerHTML = '<div class="empty-state" id="opportunitiesEmptyState">No opportunities available yet. Click "Add New Opportunity" to begin.</div>';
}
...
function renderOpportunities(opportunities) {
    ...
    if (!opportunities || opportunities.length === 0) {
        showEmptyOpportunityState();
        return;
    }
    grid.innerHTML = '';
    opportunities.forEach(opportunity => grid.appendChild(renderOpportunityCard(opportunity)));
}

async function loadOpportunities() {
    const response = await callApi('/api/opportunities', 'GET');
    if (!response.ok) {
        showToast(response.data?.error || 'Failed to load opportunities');
        return;
    }
    renderOpportunities(response.data?.data || []);
}
...
// Handle opportunity form submission
    ...
    const response = await callApi('/api/opportunities', 'POST', {
        name,
        duration,
    ...
    if (!response.ok) {
        showToast(response.data?.error || 'Unable to create opportunity');
        return;
    }
    ...
    if (grid) {
        ...
        grid.appendChild(renderOpportunityCard(response.data.data));
    }
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    const remember = document.querySelector('#loginForm .remember-me input')?.checked || false;
    const response = await callApi('/api/login', 'POST', { email, password, remember });
    if (!response.ok) {
        showToast(response.data?.error || 'Invalid email or password');
        shakeForm('loginForm');
        return;
    }

    showToast('Login successful! Redirecting...');
    setTimeout(() => showDashboard(response.data.data.email || email), 1200);
    generateCaptcha('login');
});
...
document.getElementById('signupForm').addEventListener('submit', async function(e) {
    ...
    const response = await callApi('/api/signup', 'POST', { full_name: name, email, password, confirm_password: confirmPassword });
    if (!response.ok) {
        showToast(response.data?.error || 'Unable to create account');
...
document.getElementById('forgotForm').addEventListener('submit', async function(e) {
    ...
    const response = await callApi('/api/forgot-password', 'POST', { email });
    if (!response.ok) {
        showToast(response.data?.error || 'Unable to send reset link');
        shakeForm('forgotForm');
        return;
    }

    showToast(response.data.message || 'Reset link sent to your email!');
    generateCaptcha('forgot');
    this.reset();
});
...
window.addEventListener('DOMContentLoaded', async () => {
    const response = await callApi('/api/me', 'GET');
    if (response.ok && response.data?.data?.email) {
        showDashboard(response.data.data.email);
    }
});
